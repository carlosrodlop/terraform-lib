...
variable "eks_bp_version" {
  description = "AWS EKS blueprint version."
  default     = "v5"
  type        = string

  validation {
    condition     = contains(["v4", "v5"], var.eks_bp_version)
    error_message = "AWS EKS blueprint version valid version are v4 or v5."
  }
}
